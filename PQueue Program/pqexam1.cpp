...
// also been emptied.
// **************************************************************************
bool correct(PriorityQueue<TestDataType> &test, size_t n, int items[])
// Postcondition: Some tests have been run on the test priority queue.
// If this priority queue has n items, and these items are items[0]
// through items[n-1] (in that order), then the function has printed
// "Test passed." to cout and returned true. Otherwise the function
...
// returns POINTS[1]. Otherwise the function returns zero.
{
	...
	PriorityQueue<TestDataType> test;
...
int test2()
{
	...
	PriorityQueue<TestDataType> test, empty;
	PriorityQueue<TestDataType>* pq_ptr;
	size_t base_usage;
	int i;
	int next;
	...
	pq_ptr = new PriorityQueue<TestDataType>;
	for (i = 0; i < TEST_SIZE; i++)
...
// value is zero.
{
	...
	std::cout << std::flush;
	PriorityQueue<TestDataType> copy1(test);
	...
	test.insert(3, 25);
	test.insert(4, 10);
	PriorityQueue<TestDataType> copy2(test);
...
// value is zero.
{
	PriorityQueue<TestDataType> test;
	int items[4] = { 1, 2, 3, 4 };
	char* oldbytes = new char[sizeof(PriorityQueue<TestDataType>)];
	char* newbytes = new char[sizeof(PriorityQueue<TestDataType>)];
	int i;

	std::cout << "A. Testing that assignment operator works okay for empty queue...";
	std::cout << std::flush;
	PriorityQueue<TestDataType> copy1;
	copy1.insert(1, 1);
	copy1 = test;
	if (!correct(copy1, 0, items)) return 0;
	...
	test.insert(3, 25);
	test.insert(4, 10);
	PriorityQueue<TestDataType> copy2;
	copy2 = test;
	test.insert(5, 80); // Alter the original, but not the copy
	if (!correct(copy2, 4, items)) return 0;
	...
	std::cout << std::flush;
	memcpy(oldbytes, &test, sizeof(PriorityQueue<TestDataType>));
	test = test;
	memcpy(newbytes, &test, sizeof(PriorityQueue<TestDataType>));
	for (i = 0; i < sizeof(PriorityQueue<TestDataType>); i++)
