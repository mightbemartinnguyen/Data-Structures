...
// Data type used as the template argument for the exam execution
typedef double TestDataType;
...
void* operator new(size_t size) {
	if (enableRefCount) {
		...
		std::cout << "new operator invoked with an item size of: " << size << ".\tRef Count now: " << newRefCount << std::endl;
#endif
	}
	void* p = malloc(size);
...
bool checkRefCount() {
	...
	if (::newRefCount > 0) {
		cout << "You failed to delete every item you created with new." << endl;
	}
	else {
		cout << "You failed by deleting more items than you created with new." << endl;
	}
	cout << "You are out of balance by: " << ::newRefCount << " This should be Zero." << endl;
	cout << "Please make sure you have Deleted:\n";
	cout << " - the old list (all nodes) in the assignment operator (operator=()).\n";
	cout << " - the nodes in the destructor ~sequence().\n" << endl;
	cout << " - the node of interest in remove_current().\n" << endl;
...
// **************************************************************************
// bool test_basic(const sequence<TestDataType>& test, size_t s, bool has_cursor)
...
// **************************************************************************
// bool test_items(sequence<TestDataType>& test, size_t s, size_t i, double items[])
...
// **************************************************************************
// bool correct
//   (sequence<TestDataType>& test, size_t s, size_t cursor_spot, double items[])
...
// **************************************************************************
bool correct(sequence<TestDataType>& test, size_t size, size_t cursor_spot, double items[])
{
...
{
	{
		sequence<TestDataType> empty;                // An empty sequence
		sequence<TestDataType> test;                 // A sequence to add items to
...
int test4()
{
	{
		const size_t TESTSIZE = 30;
		sequence<TestDataType> original; // A sequence that we'll copy.
		...
		sequence<TestDataType> copy1(original);
		...
		for (i = 2; i <= 2 * TESTSIZE; i++)
			original.attach(i);
		sequence<TestDataType> copy2(original);
		...
		sequence<TestDataType> copy3(original);
		original.start();
		original.advance();
		original.remove_current(); // Delete 2 from original, but not the copy.
		...
		sequence<TestDataType> copy4(original);
		original.start();
		original.advance();
		original.remove_current(); // Delete 2 from original, but not the copy.
		...
		sequence<TestDataType> copy5(original);
		original.start();
		original.advance();
		original.remove_current(); // Delete 2 from original, but not the copy.
...
int test6()
{
	{
		const size_t TESTSIZE = 30;
		sequence<TestDataType> testa, testi;
		double items[2 * TESTSIZE];
		size_t i;
...
int main()
{
	...
	cerr << "If you submit the Chapter 6 Templetes Program to Canvas, this\n";
	cerr << "part of the grade could be as high as " << static_cast <int> ((sum * 10.0f) / POINTS[0]) << " points out of 10 points.\n\n";  // Auto Grader has 100 points possible
	cerr << endl;
